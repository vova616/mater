...
func Marshal(v interface{}) ([]byte, os.Error) {
	e := &encodeState{}
	e.structIds = make(map[uintptr]int)
	err := e.marshal(v)
	if err != nil {
		return nil, err
	}
...
type encodeState struct {
	...
	//holds ids of already serialized structs,
	//allows recursive data to be serialized safely
	structIds map[uintptr]int 
	lastId int
}
...
func (e *encodeState) reflectValueQuoted(v reflect.Value, quoted bool) {
	...
	if j, ok := v.Interface().(Marshaler); ok {
		...
		if v.Kind() == reflect.Struct {
			addr := v.UnsafeAddr()
			if id, ok := e.structIds[addr]; ok {
				//only write the id, struct already serialized earlier
				e.WriteByte('{')
				e.string("$ID")
				e.WriteByte(':')
				e.reflectValue(reflect.ValueOf(id))
				e.WriteByte('}')
				return
			} else {
				//inject new id at the end of the json object
				injectId = true
			}
		}
		//
		...
		if err != nil {
			...
			//TODO: we 
			//unread the last byte, write this structs id and save it
			addr := v.UnsafeAddr()
			id := e.lastId
			e.lastId++
			//unread last byte
	...
	case reflect.Struct:
		...
		//check if this struct was already serialized
		//if so, only serialize it's id
		addr := v.UnsafeAddr()
		if id, ok := e.structIds[addr]; ok {
			e.string("$ID")
			e.WriteByte(':')
		...
		//advance e.lastId and insert it
		id := e.lastId
		e.lastId++
		e.string("$ID")
		e.WriteByte(':')
